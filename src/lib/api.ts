...
export function useCourses() {
  ...
  if (!isAuthenticated || !user?.id) {
    // Not authenticated or user info missing: show empty quickly, not loading
    setIsLoading(false);
    setCourses([]); // Clean up courses
    return;
  }
  ...
  fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/users/${user.id}/courses`)
    ...
    .then(data => {
      ...
      setIsLoading(false); // <--- Make sure to set here as well!
    })
    .catch(err => {
...
export const generateCourse = async (courseId: string, config: any) => {
...
export const reviewGeneratedContent = async (courseId: string) => {
  const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/courses/${courseId}/review`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' }
  });

  if (!response.ok) {
    throw new Error(`Failed to review course: ${response.status}`);
  }
