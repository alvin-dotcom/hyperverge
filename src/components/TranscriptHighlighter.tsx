...
export function TranscriptHighlighter({
  ...
  function getTipForSegment(seg: Segment, idx: number): string | null {
    for (const tip of tips) {
      // Accept "line 1" or "timestamp 0:05" etc.
      ...
      if (tip.reference.match(/timestamp\s*(\d+:\d+)/i)) {
        ...
        if (Math.abs(segTime - tipSec) < 2) return tip.tip; // ~2s threshold
      }
    }
    return null;
  }
  return (
    <div className="my-3 p-3 bg-slate-50 border rounded text-black">
        ...
        <div>
          {segments.map((seg, i) => {
            ...
            return (
              <span
                key={i}
                className={`px-1 ${tip ? "bg-yellow-200 underline cursor-help" : ""}`}
                title={tip || ""}
              >
